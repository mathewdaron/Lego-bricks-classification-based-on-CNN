# ============================================================
# train/train_shape.py
# 乐高形状识别模型 —— 训练脚本（V1 / V2 / V3 通用）
#
# 用法（在项目根目录下运行）：
#   python train/train_shape.py --model v1
#   python train/train_shape.py --model v2
#   python train/train_shape.py --model v3
#
# V1 专用命令（降低学习率防止梯度爆炸）：
#   python train/train_shape.py --model v1 --lr 0.0002
#
# 职责：纯训练逻辑，数据增强完全由 utils_shape/dataset.py 负责
# ============================================================
...
# ── 把项目根目录加入Python路径 ────────────────────────────────
# 原因：train_shape.py 在 train/ 子文件夹里
# 如果不加这行，Python 找不到 utils_shape/ 和 models_shape/ 文件夹
...
# ── 导入工具模块 ──────────────────────────────────────────────
# 数据加载器（含在线增强）完全由 utils_shape/dataset.py 负责
...
# ============================================================
CONFIG = {
    "data_dir"       : "train_data_shape",  # 数据集（相对项目根目录）
    "checkpoint_dir" : "checkpoints",      # 模型权重保存目录
    "results_dir"    : "results",          # 曲线图保存目录
    "batch_size"     : 64,                 # 云端GPU显存充足用64，不足改32
    "num_epochs"     : 50,                 # 三个模型统一训练50轮
    "learning_rate"  : 0.001,             # Adam初始学习率（V2/V3默认值）
    "num_classes"    : 45,                 # 形状类别数
    "num_workers"    : 4,                  # 云端AutoDL设4；本地Windows设0
}

# ★ 修改1：各模型专属默认学习率 ★
# V1 没有 BatchNorm，强数据增强下容易梯度爆炸
# 降低学习率让训练更稳定
# V2/V3 有 BN 保护，可以用更高的学习率快速收敛
MODEL_DEFAULT_LR = {
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    返回：
        avg_loss  : 本epoch平均损失值
        accuracy  : 本epoch训练准确率（百分比，如 82.3）
    """
    ...
    for images, labels in loader:
        ...
        # 前向传播：图片 → 模型 → 各类别得分
        outputs = model(images)        # shape: (batch_size, 45)
        ...
        # 对 V1（无BN）至关重要：
        #   - 没有 BN 的网络，每层激活值的分布不稳定
        #   - 遇到增强后的异常样本时，梯度可能暴涨到几百甚至上千
        #   - 一次梯度爆炸就能毁掉所有已学到的权重
        # 对 V2/V3（有BN）无害：
        #   - BN 已经把激活值归一化，梯度本来就不会太大
        #   - clip_grad_norm_ 检测到范数没超过阈值时什么都不做
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=2.0)

        # 用 Adam 更新参数
        optimizer.step()

        # 统计损失（乘以batch大小，最后再除以总数，得到真正的样本平均）
...
def save_curves(loss_list, acc_list, lr_list, model_name, results_dir):
    ...
    plt.title(f'Model {model_name.upper()} — Training Loss', fontsize=14)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    loss_path = os.path.join(results_dir, f"model_{model_name}_loss.png")
    ...
    plt.title(f'Model {model_name.upper()} — Training Accuracy', fontsize=14)
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.ylim(0, 100)
    plt.legend()
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    acc_path = os.path.join(results_dir, f"model_{model_name}_acc.png")
    ...
    plt.title(f'Model {model_name.upper()} — Learning Rate Schedule', fontsize=14)
    plt.xlabel('Epoch')
    ...
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    lr_path = os.path.join(results_dir, f"model_{model_name}_lr.png")
...
def main():
    ...
    # ── 解析命令行参数 ────────────────────────────────────────
    parser = argparse.ArgumentParser(description="乐高形状识别训练脚本")
    parser.add_argument(
        '--model',
        type=str,
        required=True,
    ...
    # 用法举例：
    #   python train/train_shape.py --model v1          → 自动用 0.0002
    #   python train/train_shape.py --model v2          → 自动用 0.001
    #   python train/train_shape.py --model v1 --lr 0.0005  → 手动覆盖
    parser.add_argument(
        '--lr',
        type=float,
    ...
    print("\n" + "=" * 60)
    print(f"  乐高形状识别 — 训练 Model {model_name.upper()}")
    print("=" * 60)
    print(f"  学习率：{CONFIG['learning_rate']}（"
          f"{'命令行指定' if args.lr is not None else '模型默认值'}）")

    # ── 路径拼接（全部基于ROOT_DIR，跨平台通用）─────────────────
    ...
    # ── 加载数据 ──────────────────────────────────────────────
    # 完全交给 utils_shape/dataset.py，train 本身零增强代码
    print(f"\n[数据] 加载中...")
    ...
    for epoch in range(1, CONFIG["num_epochs"] + 1):
        ...
        # 这样训练结束后，checkpoints/ 里存的是整个训练过程中最好的模型
        if accuracy > best_acc:
            ...
            print(f"  ✅ 新最优模型已保存（Acc: {best_acc:.2f}%）→ {save_path}")
    ...
    # 保存训练曲线图
    print("\n[结果] 保存训练曲线图...")
    save_curves(loss_history, acc_history, lr_history, model_name, results_dir)
